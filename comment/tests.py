...
class CommentCreate(APITestCase):
    ...
    def setUp(self):
        ...
        self.parent_comment = Comment.objects.create(text="içerik yorum", author=self.user, post=self.post)
        self.test_jwt_authentication()

    def test_jwt_authentication(self):
        response = self.client.post(self.login_url, data={"username": "veli", "password": "test1234"})
...
class CommentUpdateDeleteTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.comment = Comment.objects.create(text="içerik yorum", author=self.user, post=self.post)
    ...
    def test_update_comment(self):
        response = self.client.put(self.url, data= {"text": "icerik"})
        self.assertEqual(200, response.status_code)
        self.assertEqual(Comment.objects.get(pk=self.comment.id).text,"icerik")

    def test_update_comment_other_user(self):
        self.test_jwt_authentication("veli2")
        response = self.client.put(self.url, data={"content": "icerik"})
        self.assertEqual(403, response.status_code)
        self.assertNotEqual(Comment.objects.get(pk=self.comment.id).text, "icerik")
